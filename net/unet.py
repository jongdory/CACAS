...
# from net.resunet import Upsample
import torch
...
class Interpolation(nn.Module):
    ...
    def forward(self, x, size):
        # print(x, size)
        return nn.functional.interpolate(x, size=size, scale_factor=self.scale_factor, mode=self.mode)
...
class UpBlock(nn.Module):
    ...
    def __init__(
        ...
        self.interpolation = Interpolation(mode = "trilinear")
    ...
    def forward(self, encoder_layer, decoder_layer):
        ...
        up_layer = self.interpolation(up_layer, size=tuple(encoder_layer.shape[2:]))
...
@register_model
def Unet3D(pretrained=True, out_channels=2, **kwargs):
    ...
    model = UNet(
        ...
        n_blocks=5,
        start_filters=32,
        activation=ActivationFunction.LEAKY,
        normalization=NormalizationLayer.INSTANCE,
...
if __name__ == "__main__":
    ...
    # [B, C, H, W]
    summary = summary(model=unet, input_size=(1, 1, 239, 192, 231), device="cpu")